...
import { AUTH_ERROR_CODES, MARKETPLACE_ERROR_CODES } from '@sobrebox/shared';
...
describe('error catalog parity', () => {
  ...
  it('every marketplace error code has an es + en translation', () => {
    for (const code of Object.values(MARKETPLACE_ERROR_CODES)) {
      expect(es.Errors).toHaveProperty(code);
      expect(en.Errors).toHaveProperty(code);
    }
  });
